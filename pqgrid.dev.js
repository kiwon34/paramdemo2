    ...
    cTree.prototype = $.extend({}, pq.mixin.ChkGrpTree, pq.mixin.GrpTree, {
        ...
        moveNodes: function(nodes, parentNew, indx, skipHistory) {
            ...
            if (len) {
                ...
                if (TM.historyMove) skipHistory || that.iHistory.push({
                    callback: function(redo) {
                        var indxOld2 = indxOld;
                        if (parentNew == parentOld && !redo) {
                            indxOld2++
                        }
                        self.moveNodes(nodes, redo ? parentNew : parentOld, redo ? indxOrig : indxOld2, true)
                    }
                });
                DM.data = self.flatten(parentNew ? roots : dataNew);
